...
import { dietForCountry, detectCountry, productEmoji, Meal } from "@/glow/diet";
...
export default function RoutinePage() {
  ...
  // live week — Sunday..Saturday around today
  const week = useMemo(() => {
  ...
  useEffect(() => {
    ...
    const c = localStorage.getItem("velmora_country") || detectCountry();
    setCountry(c);
  }, []);
  ...
  const diet = dietForCountry(country);
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", flexDirection: "column" }}>
      ...
      <div className="glow-scroll" style={{ flex: 1, overflowY: "auto", padding: "16px 20px 130px" }}>
        {/* live day label */}
        <div style={{ fontFamily: SANS, fontSize: 15, fontWeight: 700, color: T.text, marginBottom: 14 }}>
          {DAY_NAMES[selDate.getDay()]}, {selDate.getDate()} {MONTHS[selDate.getMonth()]}
          {selDate.toDateString() === today.toDateString() && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 700, color: T.accentText, background: T.accentSoft, padding: "2px 8px", borderRadius: 99 }}>TODAY</span>}
        </div>
        ...
        {tab === "Skincare" ? (
                  ...
                  {items.map(({ idx, time, period, name, brand, pi }) => (
                    <div key={idx} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      ...
                      <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 12, padding: "11px 13px", borderRadius: 18, cursor: "pointer", background: checked[idx] ? T.surface2 : T.surface, border: "1px solid " + (checked[idx] ? T.border : rgba(SEC_COL[section], 0.18)), boxShadow: checked[idx] ? "none" : "0 3px 14px rgba(60,30,20,0.07)", opacity: checked[idx] ? 0.6 : 1, transition: "all .25s" }}>
                        ...
                        <div style={{ width: 46, height: 46, borderRadius: 13, flexShrink: 0, background: PASTELS[pi % PASTELS.length], display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>{productEmoji(name)}</div>
          ...
          <DietPlan diet={diet} country={country} onRegen={() => router.push("/coach")} />
        )}
      </div>
...
function DietPlan({ diet, country, onRegen }: { diet: { plan: Meal[]; avoid: string[]; region: string }; country: string; onRegen: () => void }) {
  return (
    <div>
      {/* region banner */}
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: 14, borderRadius: 18, marginBottom: 18, background: "linear-gradient(120deg, rgba(95,173,114,0.14), rgba(95,173,114,0.05))", border: "1px solid rgba(95,173,114,0.25)" }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: SANS, fontSize: 14.5, fontWeight: 700, color: T.text }}>{country} skin-diet plan</div>
          <div style={{ fontFamily: SANS, fontSize: 12, color: T.textMute }}>Auto-picked for your region · foods for glow</div>
        </div>
      </div>
      ...
      {diet.plan.map((meal) => (
        <div key={meal.meal} style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {meal.items.map((f) => (
              <div key={f.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 16, background: T.surface, border: `1px solid ${T.border}`, boxShadow: "0 3px 12px rgba(60,30,20,0.06)" }}>
      ...
      <PrimaryBtn icon="spark" onClick={onRegen}>Personalise with AI</PrimaryBtn>
    </div>
  );
}
