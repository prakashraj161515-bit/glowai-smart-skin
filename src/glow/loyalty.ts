...
// Rules:
// • A 30-day consecutive login streak banks ONE 10% discount.
// • The banked discount is saved forever until it's used on a purchase —
//   even if the login streak later breaks.
// • Only ONE discount can be banked at a time. A second 30-day streak does
//   NOT add another until the first is purchased.
// • Buying a (discounted) plan consumes the discount and restarts a fresh
//   30-day cycle, so completing another 30 days banks the next discount.
...
export type Loyalty = { streak: number; progress: number; banked: boolean; daysLeft: number };
...
export function tickLoyalty(): Loyalty {
  ...
  if (last === today) {
    ...
    // missed a day (or first ever) — restart progress; banked discount stays
    streak = 1; anchor = 0;
  }
  set("velmora_last_login_date", today);
  set("velmora_streak", String(streak));

  // bank a discount once 30 days of the current cycle are complete
  if (!banked && streak - anchor >= 30) {
    banked = true; anchor = streak;
    set("velmora_discount_banked", "1");
  }
  set("velmora_streak_anchor", String(anchor));

  const progress = banked ? 30 : Math.min(30, streak - anchor);
  return { streak, progress, banked, daysLeft: Math.max(0, 30 - progress) };
}
...
// Consume the banked discount on purchase and restart a fresh 30-day cycle.
